...
(function () {
    ...
    async function loadVideoFilter() {
        try {
            const resp = await fetch(BASE + '/api/processed-videos?limit=50');
    ...
    async function loadViolations() {
        ...
        let url = `${BASE}/api/violations?limit=${PAGE_SIZE}&skip=${skip}`;
    ...
    clearBtn.addEventListener('click', async () => {
        ...
        try {
            await fetch(BASE + '/api/violations', { method: 'DELETE' });
            currentPage = 0;
            loadViolations();
        } catch (e) {
