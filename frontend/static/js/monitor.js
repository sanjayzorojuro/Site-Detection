...
(function () {
    ...
    function connectWebSocket() {
        let wsUrl;
        if (BASE) {
            // Cross-origin: derive WS URL from the API base
            const backendUrl = new URL(BASE);
            const wsProtocol = backendUrl.protocol === 'https:' ? 'wss:' : 'ws:';
            wsUrl = `${wsProtocol}//${backendUrl.host}/ws/detections`;
        } else {
            // Same-origin (local dev)
            const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
            wsUrl = `${protocol}//${window.location.host}/ws/detections`;
        }
    ...
    function startFeed() {
        ...
        videoFeed.src = BASE + '/api/video-feed?' + Date.now();
        stopBtn.disabled = false;
    ...
    fileInput.addEventListener('change', async (e) => {
        ...
        try {
            const resp = await fetch(BASE + '/api/upload-video', { method: 'POST', body: formData });
    ...
    webcamBtn.addEventListener('click', async () => {
        try {
            const resp = await fetch(BASE + '/api/start-webcam', { method: 'POST' });
            const data = await resp.json();
            if (resp.ok) {
                startFeed();
            } else {
    ...
    demoBtn.addEventListener('click', async () => {
        try {
            const resp = await fetch(BASE + '/api/start-demo', { method: 'POST' });
            const data = await resp.json();
            if (resp.ok) {
                startFeed();
            } else {
